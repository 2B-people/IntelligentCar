...
Protocol::Protocol(std::shared_ptr<SerialDevice> serial_device_ptr) : running_(false),
  ...
  while (running_)
  {
    ...
    for (i = 1; i < SESSION_TABLE_NUM; i++)
    {
      ...
      if (cmd_session_table_[i].usage_flag == 1)
      {
        ...
        if ((std::chrono::duration_cast<std::chrono::milliseconds>(current_time_stamp - cmd_session_table_[i].pre_time_stamp) >
    ...
    if (container_ptr)
    {
                                                ...
                                                container_ptr->command_info.cmd_id)) == 0)
      {
        buffer_pool_map_[std::make_pair(container_ptr->command_info.cmd_set,
                                        container_ptr->command_info.cmd_id)] = std::make_shared<CircularBuffer<RecvContainer>>(100);
      }
      //1 time copy
      buffer_pool_map_[std::make_pair(container_ptr->command_info.cmd_set,
                    ...
                    void *message_data)
{
  ...
  else
  {
    ...
    // Index:这里有bug,待解决
    memcmp(command_info, &(container.command_info), sizeof(command_info));
    // LOG_INFO << "Take command of " << command_info->cmd_set << command_info->cmd_id << " does not match the command received";
    if (int(container.command_info.receiver) != int(command_info->receiver))
    {
      LOG_INFO << "Requested receiver: " << container.command_info.receiver << ", Get receiver: " << command_info->receiver;
      return false;
    }

    if (int(container.command_info.sender) != int(command_info->sender))
    {
      LOG_INFO << "Requested sender: " << container.command_info.sender << ", Get sender: " << command_info->sender;
      return false;
    }

    if (int(container.command_info.need_ack) != int(command_info->need_ack))
    {
      // LOG_INFO << "Requested need_ack: " << container.command_info.need_ack << ", Get need_ack: " << command_info->need_ack;
    }

    if (int(container.command_info.length) != int(command_info->length))
    {
      LOG_INFO << "Requested length: " << container.command_info.length << ", Get length: " << command_info->length;
      return false;
    }
    ...
    default:
      ...
      else
      {
        ...
        case ACKSessionStatus::ACK_SESSION_USING:
          ...
          memory_pool_ptr_->LockMemory();
          session_header_ptr = (Header *)ack_session_table_[header_ptr->receiver][header_ptr->session_id - 1].memory_block_ptr->memory_ptr;
